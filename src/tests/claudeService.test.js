...
import { buildGeminiPrompt, getGeminiPicks } from '../services/claudeService.js'

// buildGeminiPrompt() is pure and safe to test directly — never call the real Gemini API here.
...
describe('buildGeminiPrompt', () => {
  ...
  it('includes the group\'s most restrictive budget', () => {
    ...
    const prompt = buildGeminiPrompt(candidates, participants, [])
  ...
  it('reports no allergy restriction when none are declared', () => {
    const prompt = buildGeminiPrompt(candidates, [{ allergies: [] }], [])
...
// getGeminiPicks() never calls the real Gemini API in tests — fetch is mocked below.
describe('getGeminiPicks', () => {
  afterEach(() => {
    vi.unstubAllEnvs()
    vi.unstubAllGlobals()
  })

  function mockGeminiResponse(picks) {
  ...
  it('maps valid picks back to the matching candidate, ignoring hallucinated names', async () => {
  ...
  it('returns mapped picks when all 3 names match real candidates', async () => {
    ...
    expect(result[0].name).toBe('Le Petit Gaulois')
